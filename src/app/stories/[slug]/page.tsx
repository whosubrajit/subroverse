            ...
            ? [
                {
                  ...
                  position: 3,
                  name: story.series,
                  item: `${origin}/series/${story.seriesSlug}`,
                },
              ]
            : []),
          {
            "@type": "ListItem",
            position: story.series && story.seriesSlug ? 4 : 3,
            name: story.title,
            item: storyUrl,
          },
        ],
    ...
    <div className="min-h-screen text-[#f0ebf5]">
      ...
      <nav className="border-b border-[rgba(184,150,209,.08)] px-6 py-6">
        <div className="mx-auto flex max-w-3xl items-center justify-between">
          <Link href="/" className="font-cursive text-xl text-[#b896d1]">
            subroverse
          </Link>
          <Link
            href="/stories"
            className="text-xs uppercase tracking-[.18em] text-[#8474a0] hover:text-[#b896d1]"
          >
            all stories
          </Link>
        </div>
      ...
      <main className="mx-auto max-w-2xl px-6 pb-32 pt-14">
        <Breadcrumbs
          items={[
            ...
            ...(story.series && story.seriesSlug
              ? [{ name: story.series, href: `/series/${story.seriesSlug}` }]
              : []),
            { name: story.title },
          ]}
          className="mb-8"
          includeSchema={false}
        />
        <p className="font-cursive text-base text-[#b896d1]">
          {story.format}
          {story.series ? ` · ${story.series}` : ""}
        </p>
        <h1 className="font-display mt-3 text-4xl font-light italic leading-tight md:text-6xl">
          {story.title}
        </h1>
        {story.subtitle && (
          <p className="font-display mt-5 text-2xl italic text-[#9e90af]">
            {story.subtitle}
          </p>
        )}
        <p className="mt-6 text-xs text-[#8474a0]">
        ...
        <article lang={languages[0] || "en"} className="prose-story mt-16">
        ...
        <div className="mt-16 border-t border-[rgba(184,150,209,.1)] pt-10">
          <p className="font-cursive text-center text-[#b896d1]/60 text-xl mb-12">
            — written with love
          </p>
              ...
              {(() => {
                ...
                if (nextStory) {
                  return (
                    ...
                    >
                      <p className="font-display italic text-2xl text-[#9e90af] group-hover:text-[#b896d1] transition-colors">
                        ← {nextStory.title}
                      </p>
                    </Link>
                  )
          ...
          <div className="text-center mt-12">
            <Link
              href="/stories"
              className="inline-flex min-h-11 items-center text-xs uppercase tracking-[.18em] text-[#8474a0] hover:text-[#b896d1] transition-colors"
            >
              back to stories
            </Link>
          </div>
