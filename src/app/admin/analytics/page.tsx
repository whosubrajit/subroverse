...
import { publicStoryCondition } from "@/lib/story-publication"
...
export default async function AnalyticsPage() {
  ...
  if (process.env.DATABASE_URL) {
    ...
    const [published, audience, sent, messages] = await Promise.all([
      db.select({ value: count() }).from(stories).where(publicStoryCondition(stories)),
  ...
  return <SectionShell eyebrow="the bigger picture" title="Analytics" description="Cloudflare zone traffic, alongside publishing and reader growth.">
