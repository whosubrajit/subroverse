...
const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Keep the CSS gradient visible on devices without WebGL2.
      return;
    }
    ...
    // Listen on the window: this decorative layer must never intercept links.
    ...
    const tryStart = () => {
      if (!reducedMotion && !contextLost && isVisible && isPageVisible && raf === 0) raf = requestAnimationFrame(loop);
    };
    const tryStop = () => {
      if (raf !== 0) {
    ...
    const onMotionChange = () => {
      ...
      program.uniforms.uEnableMouse.value = enableMouseRef.current && !reducedMotion;
      if (reducedMotion) {
        tryStop();
  ...
  useEffect(() => {
    ...
    u.uEnableMouse.value = mouseInteraction && !window.matchMedia("(prefers-reduced-motion: reduce)").matches;
