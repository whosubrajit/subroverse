...
export const storyInputSchema = z
  ...
  .refine(
    (story) => story.status !== "scheduled" || Boolean(story.scheduledFor),
    {
...
export async function readPublicStories({
  ...
  return getDb()
    .select()
    .from(stories)
    .where(publicStoryCondition(stories, now))
    .orderBy(
      ...(featuredFirst ? [desc(stories.featured)] : []),
      desc(publicationDateColumn(stories)),
      desc(stories.id),
    )
}
...
export function toPublicStory(story: typeof stories.$inferSelect): PublicStory {
  return {
    ...
    date: (publicationDate(story) ?? story.createdAt).toLocaleDateString(
      "en-US",
      {
        timeZone: "Asia/Dhaka",
        year: "numeric",
...
export async function readPublicStoryFeed(
  options: Parameters<typeof readPublicStories>[0] = {},
) {
...
export async function findPublicStory(slug: string, now = new Date()) {
  ...
  const [story] = await getDb()
    .select()
    .from(stories)
    .where(and(eq(stories.slug, slug), publicStoryCondition(stories, now)))
    .limit(1)
  return story ?? null
}
...
export async function findPublicSeries(slug: string) {
  ...
  const [bySlug] = await db
    .select({ series: stories.series })
    .from(stories)
    .where(and(eq(stories.seriesSlug, slug), visible))
    .limit(1)
